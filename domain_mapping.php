...
Plugin URI: https://github.com/youknow0/wordpress-mu-domain-mapping
Description: Map any blog on a WordPress website to another domain and have the admin backend served over HTTPS.
Version: 0.1
Author: Nico Boehr
*/
...
function redirect_to_mapped_domain() {
	...
	// don't redirect template previews
...
## mod
function dm_rewrite_login_url($url) {
        global $current_blog;
        ...
        $url = get_original_url( 'siteurl', $current_blog->blog_id ) . substr( $url, $pos );
        return $url;
 
}
add_filter( 'login_url', 'dm_rewrite_login_url' );
function dm_force_secure_logged_in_cookie( $secure ) {
	return true;
}
add_filter('secure_logged_in_cookie', 'dm_force_secure_logged_in_cookie', 10, 1);
 
function dm_force_secure_auth_cookie( $secure ) {
	return true;
}
add_filter('secure_auth_cookie', 'dm_force_secure_auth_cookie', 10, 1);
 
function dm_force_secure_signon_cookie( $secure ) {
	return true;
}
add_filter('secure_signon_cookie', 'dm_force_secure_signon_cookie', 10, 1);
